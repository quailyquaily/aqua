...
## Objective
Simplify Aqua's implementation to the minimum required for secure, reliable MAEP operation:
- trustworthy identity and peer verification
- stable hello + RPC connectivity
- durable inbox/outbox/audit persistence
...
- related tests in `maep/*_test.go` and `internal/fsstore/*_test.go`
...
Problem:
`agent.data.push` currently enforces a JSON envelope (`message_id`, `text`, `sent_at`) in core transport handling.

Changes:
- Make payload opaque at MAEP transport level.
...
- Keep CLI convenience for `--text`, but treat it as user payload construction only.
...
- `maep/jsonprofile.go` (possibly deleted)
